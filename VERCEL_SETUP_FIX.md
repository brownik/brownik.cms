...
## 문제 상황
Vercel 대시보드에서 `townE-backend`와 `new_analysis_docs`만 보이고 `townE`가 보이지 않는 경우
...
1. `townE`의 내용을 루트로 이동
2. 또는 Vercel에서 `townE` 폴더만 별도 프로젝트로 연동
...
1. Vercel 대시보드 → Settings → General
2. Root Directory: `townE` 입력
3. Save
4. Deployments → Redeploy

이렇게 하면 Vercel이 `townE` 폴더를 프로젝트 루트로 인식합니다.
